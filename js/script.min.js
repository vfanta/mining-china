...
$( document ).ready(function() {
    ...
    var $contactForm = $('.feedback__form');

    $contactForm.submit(function(e) {
        e.preventDefault();

        $.ajax({
            url: 'https://formspree.io/vfanta1@gmail.com',
            method: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            beforeSend: function() {
                $contactForm.append('<div class="alert alert--loading">Сообщение отрпавляется…</div>');
                $('.alert--loading').fadeIn(1000).fadeOut(1000, function(){$(this).remove()});
            },
            success: function(data) {
                $contactForm.find('.alert--loading').hide();
                $contactForm.append('<div class="alert alert--success">Успешно отправлено!</div>');
                //$('.alert--succes').hide();
                $('.alert--succes').fadeIn(1000).fadeOut(5000, function(){$(this).remove()});

            },
            error: function(err) {
                $contactForm.find('.alert--loading').hide();
                $contactForm.append('<div class="alert alert--error">Ой, попробуйте еще раз.</div>');
                //$('.alert--error').hide();
                $('.alert--error').fadeIn(1000).fadeOut(5000, function(){$(this).remove()});
